...
class parser
{
    ...
    private function _modules() {
        $this->data->input = preg_replace_callback('#<!--\s*MODULE\s*(.+?)(\sAS\s*(.+?))?\s*(\sVARS\s*(.+?))?\s*-->(.*?)<!--\s*END MODULE\s*(\\1|\\3)\s*-->#si', array($this->classname, '_module_content'), $this->data->input);
        if (preg_match('#<!--\s*MODULE\s*(.+?)(\sAS\s*(.+?))?\s*(\sVARS\s*(.+?))?\s*-->(.*?)<!--\s*END MODULE\s*(\\1|\\3)\s*-->#si', $this->data->input) > 0)
            $this->_modules();
    }

    function _module_content($matches) {
        list($body,$url,,$as,,$vars,$content,) = $matches;
        if($as !== '' AND is_string($as)){
            $array_name = $as;
        }
        if($vars !== '' AND is_string($vars)){
            ...
            foreach ($array_vars as $value) {
                $ii = $i++;
                $thisvar[$ii] = explode('=', $value);
                if ( strpos( $thisvar[$ii][1], '{array:') !== FALSE )
		{
			$ret = unserialize( urldecode( substr( $thisvar[$ii][1], 7, -1 ) ));
		}
		else
		{
			$ret = $thisvar[$ii][1];
		}
                $data[$thisvar[$ii][0]] = $ret;                                
            }
            
        }
        
        $module = Modules::run($url,$as,$data);
        //print_r($this->input->data);
        if(is_array($module)){
            //$this->input->data = array_merge($this->input->data,$module);
            $mod['config'] = $this->input->config;        
            $mod['config']['show'] = FALSE;
            $mod['config']['is_string'] = TRUE;
            
            $_parser = new $this->classname;

            $return = $_parser->parse($content, $module, $mod['config']);
    ...
    private function _blocks() {
        $this->data->input = preg_replace_callback('#<!--\s*BLOCK\s*(.+?)\s*-->(.*?)<!--\s*END BLOCK\s*(\\1)\s*-->#si', array($this->classname, '_block_content'), $this->data->input);
        if (preg_match('#<!--\s*BLOCK\s*(.+?)\s*-->(.*?)<!--\s*END BLOCK\s*(\\1)\s*-->#si', $this->data->input) > 0)
            $this->_blocks();        
    }
    function _block_content($matches) {
        list($body,$block,$content,$blockname) = $matches;
        if(!@$this->input->data['block'][$block]){
            return $content;
        }else{
            return @$this->input->data['block'][$block];
        }
	...
	**/
	...
	function _cycle_content($matches)
    {
		...
		$subject = $this->_append( $subject, $this->input->data + array('T_Folder' => $this->config->folder ));
                $subject = $this->_append( $subject, $this->input->data + array('base_url' => base_url() ));
                $subject = $this->_append( $subject, $this->input->data + array('site_url' => base_url().$this->CI->config->item('language_abbr').'/' ));
                $subject = $this->_append( $subject, $this->input->data + array('theme_url' => base_url('themes/'.$this->CI->config->item('theme')).'/'));
                $subject = $this->_append( $subject, $this->input->data + array('current_lang' => $this->CI->config->item('language')));
                $subject = $this->_append( $subject, $this->input->data + array('current_lang_abbr' => $this->CI->config->item('language_abbr')));;
                //$subject = $this->_append( $subject, $this->input->data + array('last_page_url' => $CI->session->last_page(1)));
    ...
    {    	
        ...
        $this->input->data['base_url'] = base_url() ;
        $this->input->data['site_url'] = base_url().$this->CI->config->item('language_abbr').'/' ;
        $this->input->data['theme_url'] = base_url('themes/'.$this->CI->config->item('theme')).'/';
        $this->input->data['current_lang'] = $this->CI->config->item('language');
        $this->input->data['current_lang_abbr'] = $this->CI->config->item('language_abbr');
        //$this->input->data['last_page_url'] = $CI->session->last_page(1);
	...
	private function _default()
	{
		...
		$this->config->disable_includes = FALSE;
                $this->config->disable_blocks = FALSE;
                $this->config->disable_modules = FALSE;
		...
		$this->config->show = TRUE;
                $this->config->removesamples = TRUE;
		$this->config->suffix_theme_only = TRUE;
		$this->config->template_suffix = 'html';
		$this->config->theme = $this->CI->config->item('theme');
